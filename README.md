...
- **灵活页面布局**：分镜根据剧情节奏选择网格、宽主格配小格、上下主格、左右分栏或不对称版式，并明确每格位置、尺寸与阅读顺序。
- **结构化漫画文字**：分镜可按叙事需要生成对白、标题、旁白和拟声词，分别绑定格号、说话者、位置、样式与准确原文；插件按官方规则把唯一 `Text:` 块放在 Prompt 绝对末尾。明确要求纯画面时才禁用文字。
...
- **可靠图片交付**：插件直接发送不带引用的纯图片；NapCat 回执超时时核验最近消息，无法确认时不自动重复发送，并保留请求级状态供 `/n5 重发` 使用。
