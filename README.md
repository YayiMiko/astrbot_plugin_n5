...
- **Vibe 参考图**：`/n5 生成` 附图或引用图片时自动作为 Vibe 参考（最多 16 张，强度总和按官方建议 ≤1.0）；参考图编码与超量会消耗 Anlas，默认仅 AstrBot 管理员可用，可在配置页用 `reference_open_to_all` 向所有人开放。
...
| `reference_open_to_all` | 参考图是否对所有人开放；关闭时仅管理员可用 | `false` |
