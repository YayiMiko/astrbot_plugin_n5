...
4. 名招、名场面、构图、地点和道具引用走独立的联网视觉研究，形成英文可视蓝图；随后把描述与蓝图交给 DeepSeek 规划器，输出严格单行 JSON：`{"ok":true,"prompt":"...","character_prompts":{...},"error":null}`。创作引用不套用固定 Tag 数量、人物 Tag 最低项数或固定句数，`max_prompt_length` 仅作为接口字符安全上限。
