...
- **人物库**：群成员可保存「全局人物」，命中角色名时自动注入固定身份与固有外观，再由规划器补全本图服装、道具与动态。人物库为共享池：群聊与私聊共用一套，任何人创建后所有人可用（`/n5 漫画` 同样生效）。
- **画师串**：预设与切换画师串（`artist:` 风格串），由插件在最终 Prompt 前独立拼接；规划器只生成主 Prompt，不生成或修改画师串。
- **全局 NSFW 语义方向**：默认 safe 安全模式（`rating:safe`，同时删除用户自带的 `rating:` 分级词）；每人可用 `/n5 nsfw [开|safe]` 切换到 NSFW 方向词。
- **固定尺寸**：默认 832x1216 竖图；单次请求尾缀横图 / 方图 / 超宽屏分别使用 1216x832 / 1024x1024 / 1536x640，均不超免费像素上限。
- **多格漫画**：`/n5 漫画` 先规划 1–4 格分镜（景别/机位/台词分配），再按 Base 只画框、角色全进槽位的规则生成；分镜方法来自某单机游戏爱好者的 NAI5 SKILL（`nai5-prompt-expert`），人物库角色沿用固定身份，单页超 4 格会拒绝并提示拆页。
- **表情包**：`/n5 表情包 <情绪> [配字 <文字>]` 生成方形 Q 版表情包（固定 1024 白底）；配字超 8 字会被拒绝，不给配字时为纯表情。
- **画风推荐**：`/n5 画风推荐 <风格描述>` 由规划器推荐画师融合串（最多 4 位）与风格词（画师过 Danbooru 画师库校验，默认只推 P 站高人气二次元画师），下一次生成（生成/漫画/表情包通用）自动填入画师槽位，用后清空。
- **V5 模型切换**：WebUI 下拉设置默认模型，聊天中可按用户切换 V5C / V5F。
- **免费生成防护**：Opus 身份校验、总像素上限、Steps 上限、响应大小上限，以及 429 排队重试。
- **队列与并发控制**：串行生成 + 排队计数，避免并发打爆账号。
- **可靠图片交付**：插件直接发送不带引用的纯图片；NapCat 回执超时时核验最近消息，无法确认时不自动重复发送。
...
> 当前版本 `0.4.0`，要求 AstrBot `>=4.26, <5`，支持平台 `aiocqhttp`（QQ OneBot）。
...
| `nsfw_group_ids` | NSFW 白名单群号；仅名单群可用 nsfw+V5F 且无额外负面 | `[]` |
...
> 说明：`prompt_planner_provider_id` 与群聊默认模型相互独立。它必须支持原生图片输入（附图即参考）。

---

## 认证

插件需要一个 **NovelAI 持久化 API Token（PAT）**，按以下优先级读取，配一种即可：

1. 插件配置页的 `novelai_api_token`（推荐，可视化修改即时生效）；
2. 环境变量 `NOVELAI_API_TOKEN`（推荐，Linux / 容器）：

```bash
export NOVELAI_API_TOKEN="pst-你的token"
```

3. Windows DPAPI 加密文件（避免明文落盘）：
...
| `/n5 表情包 <情绪> [配字 <文字>]` | 生成方形 Q 版表情包，配字限 8 字 |
| `/n5 画风推荐 <风格描述>` | 推荐画师融合串（最多 4 位）与风格词（默认 P 站高人气二次元画师），下一次生成自动使用，用后清空 |
| `/n5 nsfw [开\|safe]` | 查看或切换 NSFW/safe，不带参数时取反（默认 safe；默认安全组内强制 safe） |
...
| `/n5 保存画风 <名称>` | 将你上次生图实际使用的画风存为画师串 |
| `/n5 创建人物 <角色名> <Prompt> [--负面 <内容>]` | 保存全局人物，命中名字时自动引用 |
| `/n5 删除人物 <角色名>` | 删除全局人物 |
| `/n5 确认` | 在 60 秒内确认「覆盖 / 删除人物」操作 |

区别：`/n5 添加画师串` 存的是你手写的内容（tag 是否有效由你保证）；`/n5 保存画风` 存的是你上次生图**实际生效**的画师槽位原样（已验证生效，但可能混有推荐附带的风格词）。两者都存入当前对话的画师串库（群共享，私聊私有）。

---

## 权限控制

插件默认向所有人开放生图权限，但群聊默认走安全策略：

- `allowed_sender_ids` 留空时任何 QQ 都能执行 NovelAI 指令（私聊和群聊统一校验）；填入名单后只允许名单中的账号执行。
- 群聊默认开放；`allow_group=false` 时禁止群聊使用。
- **群安全策略**：不在 `nsfw_group_ids` 名单的群强制 safe + V5C，并追加 `nude, naked, sex, explicit` 负面；名单群才尊重个人 nsfw 开关和模型选择且无额外负面。私聊始终按个人设置。
...
- **图片生成后没有出现**：插件会在 NapCat ACK 超时时核验最近消息；无法确认时为避免重复图片不会自动补发，请检查聊天记录确认是否已送达，或稍后重新生成。
