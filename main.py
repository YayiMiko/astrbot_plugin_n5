...
# Fixed emote sticker spec: square canvas, chibi proportions, plain background.
EMOTE_SIZE = (1024, 1024)
EMOTE_STYLE_TAGS = (
    "chibi, super deformed, upper body, white background, simple background"
)
EMOTE_NO_TEXT_NEGATIVES = (
    "text",
    "captions",
    "speech bubbles",
    "subtitles",
...
class NovelAIWebPlugin(star.Star):
    ...
    def _load_api_token(self) -> str:
        ...
        Priority: plugin config page ``novelai_api_token`` first, then the
        ``NOVELAI_API_TOKEN`` environment variable, then the Windows DPAPI file.
        ...
        """
        token = str(self.config.get("novelai_api_token", "") or "").strip()
        if not token:
        ...
        if not token:
            ...
            if os.name != "nt":
                raise NovelAIWebError(
                    f"未配置 NovelAI PAT；请在插件配置页填写 novelai_api_token，或配置 {NOVELAI_PAT_ENV}。"
                )
            try:
                encrypted = token_path.read_bytes()
            except FileNotFoundError as exc:
                raise NovelAIWebError(
                    f"未找到 NovelAI PAT；请在插件配置页填写 novelai_api_token，或配置 {NOVELAI_PAT_ENV}。"
                ) from exc
            except OSError as exc:
    ...
    def _check_access(
        ...
        *,
        ...
        """Apply sender authorization in private chats and groups.

        An empty sender whitelist opens generation to everyone; a non-empty
        list restricts usage to the listed QQ accounts.
        """
        ...
        if not allow_group_access or not bool(self.config.get("allow_group", True)):
    ...
    @staticmethod
    def _normalize_nsfw_selection(value: str) -> bool:
    ...
    async def _user_nsfw_enabled(
        ...
        """Read or persist this QQ user's NSFW direction switch.
        ...
        async with self._artist_state_lock:
            ...
            if user_state is None:
                user_state = self._new_user_state()
                state["users"][sender_id] = user_state
            user_state["nsfw_enabled"] = nsfw_enabled
            self._save_artist_state(state)
            return nsfw_enabled
    ...
    async def _deliver_generated_image(
        ...
        *,
        ...
        notice = (
            ...
            else "图片已经生成，但 QQ 图片发送失败，请稍后重新生成。"
        )
        try:
    ...
    async def generate_image(self, event: AstrMessageEvent, prompt: GreedyStr):
        ...
        if subcommand == "角色":
            ...
            try:
                self._check_access(event)
                if arguments:
                    nsfw_enabled = await self._user_nsfw_enabled(event, arguments)
                else:
        ...
        if emote_mode:
            generation_size = EMOTE_SIZE
            emotion_text, caption_separator, caption_text = prompt_text.partition(
                "配字"
            )
            if caption_separator:
                prompt_text = emotion_text.strip(" ,，;；:：")
                emote_caption = (
                    re.sub(r"\s+", " ", caption_text.strip(' ,，;；:："“”'))
                    .strip()
                    .replace('"', "“")
                )
        ...
        try:
            ...
            if emote_mode and len(emote_caption) > EMOTE_MAX_CAPTION_CHARS:
                raise NovelAIWebError(
                    f"配字过长，请控制在 {EMOTE_MAX_CAPTION_CHARS} 字以内。"
                )
            ...
            try:
                async with self._generation_semaphore:
                    if comic_mode:
                        ...
                        character_negative_prompts = tuple("" for _ in comic_captions)
                    ...
                    if emote_mode:
                        prompt_text = f"{prompt_text}, {EMOTE_STYLE_TAGS}"
                        if emote_caption:
                            prompt_text += "\nText: " + emote_caption
                        else:
                            prompt_text += ", no text"
                            ...
                            negative_items.extend(
                                item
                                for item in EMOTE_NO_TEXT_NEGATIVES
                                if item.casefold() not in present_negative
                            )
                            negative_prompt = ", ".join(negative_items)
                    if selected_artist is not None:
    ...
    async def _generate_from_api(
        ...
        *,
        ...
        """
        prompt = self._apply_global_nsfw_prompt(prompt, apply_nsfw)
