...
默认面向 NovelAI Diffusion V5 Curated。当前 AstrBot 插件会单独拼接用户选中的画师串，因此不要生成 `artist:` 标签、画师名、年份风格串或 `artist collaboration` 控制词。普通场景使用“紧凑标签块 + 1–3 句具体英文场景描述”的混合格式，利用 V5 对空间关系、互动、背景、材质、尺度和共享光照的自然语言理解。
